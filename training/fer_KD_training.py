...
def train_teacher_model(model, train_loader, val_loader, device, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'KD_pretrained_teacher_model.pth')
...
def train_student_model_with_distillation(teacher_model, student_model, train_loader, val_loader, 
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(student_model.state_dict(), 'KD_best_student_model.pth')
...
def main():
    ...
    teacher_model.load_state_dict(torch.load('KD_pretrained_teacher_model.pth'))
    ...
    student_model.load_state_dict(torch.load('KD_best_student_model.pth'))
